...
export default function CaptainTournamentDetail() {
  ...
  const [registrationStatus, setRegistrationStatus] = useState<'No Registrado' | 'Pendiente' | 'Aceptado' | 'Rechazado'>('No Registrado');
  ...
  const handleRegistration = async () => {
    ...
    Alert.alert('Inscripción enviada', 'Tu solicitud de inscripción está pendiente de aprobación.');
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {registrationStatus === 'Pendiente' && (
        <Text style={styles.pendingText}>Tu inscripción está pendiente de aprobación.</Text>
      )}

      {registrationStatus === 'Aceptado' && (
        <Text style={styles.acceptedText}>¡Tu equipo está inscrito en este torneo!</Text>
      )}

      {registrationStatus === 'Rechazado' && (
        <Text style={styles.rejectedText}>Tu inscripción ha sido rechazada.</Text>
      )}
    </View>
  );
}
